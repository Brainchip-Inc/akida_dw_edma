...
ifneq ($(word 1,$(AKIDA_KERNEL_VERSION_RANK)), 4.14)
$(error Kernel $(VERSION).$(PATCHLEVEL) too old)
else ifneq ($(word 2,$(AKIDA_KERNEL_VERSION_RANK)), 4.16)
ccflags-y += -I$(src)/kernel/4.14/drivers/dma
ccflags-y += -I$(src)/kernel/4.14/include
else ifneq ($(word 3,$(AKIDA_KERNEL_VERSION_RANK)), 5.2)
ccflags-y += -I$(src)/kernel/4.16/drivers/dma
ccflags-y += -I$(src)/kernel/4.16/include
else ifneq ($(word 4,$(AKIDA_KERNEL_VERSION_RANK)), 5.3)
ccflags-y += -I$(src)/kernel/5.2/drivers/dma
ccflags-y += -I$(src)/kernel/5.2/include
else ifneq ($(word 5,$(AKIDA_KERNEL_VERSION_RANK)), 5.4)
ccflags-y += -I$(src)/kernel/5.3/drivers/dma
else ifneq ($(word 6,$(AKIDA_KERNEL_VERSION_RANK)), 5.6)
ccflags-y += -I$(src)/kernel/5.4/drivers/dma
else ifneq ($(word 7,$(AKIDA_KERNEL_VERSION_RANK)), 5.7)
ccflags-y += -I$(src)/kernel/5.6/drivers/dma
else ifneq ($(word 8,$(AKIDA_KERNEL_VERSION_RANK)), 5.16)
ccflags-y += -I$(src)/kernel/5.7/drivers/dma
else ifneq ($(word 9,$(AKIDA_KERNEL_VERSION_RANK)), 5.17)
